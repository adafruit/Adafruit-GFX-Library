 ...
 * Adafruit_GFX</a> and <a href="https://github.com/adafruit/Adafruit_BusIO">
...
/*!
    ...
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
Adafruit_MonoOLED::Adafruit_MonoOLED(uint16_t w, uint16_t h,
...
/*!
    ...
    @param  color
            Pixel color, one of: MONOOLED_BLACK, MONOOLED_WHITE or MONOOLED_INVERT.
    @return None (void).
...
void Adafruit_MonoOLED::drawPixel(int16_t x, int16_t y, uint16_t color) {
  if((x >= 0) && (x < width()) && (y >= 0) && (y < height())) {
    ...
    switch(color) {
     case MONOOLED_WHITE:   buffer[x + (y/8)*WIDTH] |=  (1 << (y&7)); break;
     case MONOOLED_BLACK:   buffer[x + (y/8)*WIDTH] &= ~(1 << (y&7)); break;
     case MONOOLED_INVERSE: buffer[x + (y/8)*WIDTH] ^=  (1 << (y&7)); break;
    }
  }
}
...
/*!
    ...
    @param  y
            Row of display -- 0 at top to (screen height -1) at bottom.
    @return true if pixel is set (usually MONOOLED_WHITE, unless display invert mode
            is enabled), false if clear (MONOOLED_BLACK).
...
*/
uint8_t *Adafruit_MonoOLED::getBuffer(void) {
  return buffer;
}
...
/*!
    ...
    @note   This has an immediate effect on the display, no need to call the
            ...
            enabled, drawing MONOOLED_BLACK (value 0) pixels will actually draw white,
            MONOOLED_WHITE (value 1) will draw black.
*/
